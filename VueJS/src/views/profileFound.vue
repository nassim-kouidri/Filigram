...
<div class="wrapper">
  <div class="profile-card js-profile-card">
    <div class="profile-card__img">
      <img :src="this.account.profilePicture" alt="profile card">
    </div>
    <div class="settings">
        <svg  v-on:click="SignalAccount()" xmlns="http://www.w3.org/2000/svg" width="45" height="80" fill="currentColor" class="bi bi-exclamation-triangle" viewBox="0 0 16 16">
  ...
  <path d="M7.002 12a1 1 0 1 1 2 0 1 1 0 0 1-2 0zM7.1 5.995a.905.905 0 1 1 1.8 0l-.35 3.507a.552.552 0 0 1-1.1 0L7.1 5.995z"/>
    ...
    <div class="profile-card__cnt js-profile-cnt">
      <div class="profile-card__name">{{account.fullname}}</div>
      <h1>{{ this.account.mail }}</h1>
      <h2>{{ this.account.pseudo }}</h2>


      <div class="profile-card__txt"> Pseudo: <strong>{{this.account.pseudo}}</strong></div>
      <div class="profile-card-loc">
        ...
        <span class="profile-card-loc__txt">
          Intech, Ivry sur Seine
        </span>
      </div>

      <div class="profile-card-inf">
        <div class="profile-card-inf__item">
          <div class="profile-card-inf__title">{{account.nbrFollowers}}</div>
          <div class="profile-card-inf__txt">Followers</div>
        </div>

        <div class="profile-card-inf__item">
          <div class="profile-card-inf__title">{{account.nbrSubscriptions}}</div>
          <div class="profile-card-inf__txt">Following</div>
        </div>

        <div class="profile-card-inf__item">
          <div class="profile-card-inf__title">{{account.nbrPosts}}</div>
      ...
      <div class="profile-card-ctr">
        ...
        <!-- <table width="900" border="1" cellspacing="1" cellpadding="5"> -->
...
body {
    min-height: 100vh;
    // background: linear-gradient(to bottom, #000428, #49010a);
}
...
import accountGetService from "../service/accountGetService";
...
export default {
  ...
  methods: {
    getAccount() {
      ...
      accountGetService.getAccountFound(this.idProfile).then((response) => {
        this.account = response.data;
        console.log(this.account.visibilityAccount);
        this.visibility = this.account.visibilityAccount;
		// console.log(this.account.id);
            this.verifVisibilityAccount();
      });
    },
    verifVisibilityAccount(){
        if(this.account.visibilityAccount == true){
            console.log("C'est bon on peut voir les posts");
            this.getPostsbyAccount();
        }else{
            console.log("Se compte ne permet de voir les posts"+this.account.visibilityAccount);
        }
    },
      getPostsbyAccount() {
        axios
        .get('http://localhost:8081/api/Account/'+this.idProfile+'/posts')
        .then((response) => {
        ...
        SignalAccount(){
          const body = {
            "signalAccount": true,
             "signalAccountOnce" : true
          };
          axios.
          put('http://localhost:8081/api/PutAccount/'+this.idProfile, body)
          .then((response) => {
            console.log("The account has been signaled");
          });
        },
